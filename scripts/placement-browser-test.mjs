...
try {
 for(const outcome of ['accept','reject','lost-reply','stale']){
  ...
  const game=createGame([{id:'a',name:'Willow'},{id:'b',name:'Ochre'}],()=>0);game.phase='play';game.hand=['wind','wind','earth'];game.bag.wind-=2;game.bag.earth--;game.movesLeft=2;
  ...
  await page.route('**/api/command',async route=>{
   const input=route.request().postDataJSON();ids.push(input.requestId);
   if(ids.length===1){
  ...
  if(outcome==='accept'||outcome==='lost-reply'){
