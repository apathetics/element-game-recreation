...
function header() {
  return `<header class="site-header"><button class="brand" data-do="home" aria-label="Element home">${icon('mark')}<span>ELEMENT<small>A GAME OF BALANCE</small></span></button><div class="header-actions">${feedbackSettings()}${room ? `<span class="connection ${offline ? 'disconnected' : ''}"><i></i>${mode === 'local' ? 'Shared device' : offline ? 'Reconnecting…' : `Table ${esc(room.code)}`}</span>` : '<span class="quiet desktop-only">2–4 PLAYERS · 30–60 MIN</span>'}<button class="text-button" data-do="rules">How to play <span class="question">?</span></button></div></header>`;
}
...
function clockSettings() {
  ...
  return `<div class="clock-settings"><label class="field-label" for="clock-minutes">TIME PER PLAYER</label><select id="clock-minutes" ${!host || busy ? 'disabled' : ''}>${CLOCK_MINUTES.map(value => `<option value="${value}" ${value === minutes ? 'selected' : ''}>${value ? `${value} minutes` : 'No timer'}</option>`).join('')}</select><p class="form-note">${minutes ? 'Runs throughout your turn, including drawing. Running out gives victory to the player targeting your Sage.' : host ? 'Play at your own pace.' : 'The table creator chooses the timer.'}</p></div>`;
}
function lobbyMarkup() {
  const isHost = room.host === me || mode === 'local';
  return `<main class="lobby"><div class="lobby-heading"><div class="eyebrow">YOUR PRIVATE TABLE</div><h1>Gather your Sages.</h1></div><div class="lobby-card">${mode === 'online' ? `<div class="invite-box"><span class="field-label">INVITE CODE</span><strong>${esc(room.code)}</strong><button class="button subtle" data-do="copy">${icon('link')} Copy invite link</button></div>` : ''}<div class="lobby-columns"><section class="lobby-options">${clockSettings()}<p class="form-note">${room.members.length} of ${room.capacity} Sages have arrived.</p>${isHost ? `<button class="button primary full" data-do="start" ${busy || room.members.length !== room.capacity ? 'disabled' : ''}>Begin the game ${icon('arrow')}</button>` : '<div class="waiting-message">Waiting for the table creator to begin…</div>'}<button class="text-button" data-do="leave" ${busy ? 'disabled' : ''}>Leave table</button></section><div class="seats">${Array.from({ length: room.capacity }, (_, i) => { const m = room.members[i]; return `<div class="seat ${m ? 'occupied' : ''}">${sage({ color: COLORS[i] })}<span>${m ? esc(m.name) : 'Waiting for a Sage…'}<small>${m ? m.id === room.host ? 'TABLE CREATOR' : 'READY TO PLAY' : 'OPEN SEAT'}</small></span>${m ? icon('check') : '<span class="waiting-dot"></span>'}</div>`; }).join('')}</div></div></div></main>`;
}
...
function controlsMarkup(g) {
  if (openingActive(g)) return '<aside class="controls opening-wait"><h2>Choosing the first Sage…</h2></aside>';
...
function tableSidebar(g) {
  ...
  return `<aside class="table-sidebar" aria-label="Table activity"><div class="turn-panel">${controlsMarkup(g)}</div><section class="shared-draw"><div class="eyebrow">${draw ? current ? 'THIS TURN’S DRAW' : 'PREVIOUS DRAW' : 'AT THE BAG'}</div><h2>${draw ? `${esc(name)} drew` : `${esc(active().name)} is choosing`}</h2><div class="drawn-stones" aria-label="Drawn stones">${stones.map(element => { const index = current ? remaining.indexOf(element) : -1; if(index >= 0) remaining.splice(index,1); return `<span class="draw-stone ${index < 0 ? 'played' : ''}" aria-label="${element}${index < 0 ? ', played' : ', remaining'}">${stone({element,ids:[0]})}${index < 0 ? '<span class="played-check">✓</span>' : ''}</span>`; }).join('') || `<span class="draw-empty">${draw ? 'No stones · 5 moves' : 'Stones will appear here.'}</span>`}</div></section><section class="history" aria-label="Move log"><h2>Move log</h2><ol>${g.log.slice().reverse().map(item => `<li>${esc(item)}</li>`).join('')}</ol></section></aside>`;
}
function gameMarkup(g) {
  return `<main class="game-layout" ${openingActive(g) ? 'inert' : ''}><div class="game-heading"><div><div class="eyebrow">${g.phase === 'finished' ? 'THE GAME IS COMPLETE' : `TURN ${String(g.turn).padStart(2, '0')}`}</div><h1>${g.phase === 'finished' ? 'A new balance.' : mine() ? `${esc(active().name)}’s turn` : 'The elements are in motion.'}</h1></div><div class="game-heading-actions">${mode === 'online' ? '<button class="text-button" data-do="copy">Invite link</button>' : '<span class="local-tag">PASS & PLAY</span>'}<button class="text-button" data-do="rules">Element guide</button></div></div>${rosterMarkup(g)}<section class="board-section">${boardMarkup(g)}<div class="board-legend"><span>${icon('earth')} Outlined earth belongs to a protected range</span></div>${lastMoveMarkup()}</section>${tableSidebar(g)}</main>`;
}
...
function openingActive(g = game()) { return g?.opening && tableNow() < g.opening.readyAt; }
function openingMarkup(g) {
  if (!openingActive(g)) return '';
  const winner = g.players.find(p => p.id === g.opening.winnerId), two = g.players.length === 2;
  const elapsed = Math.max(0, tableNow() - g.opening.startedAt);
  const front = two ? g.players[0] : winner, back = two ? g.players[1] : winner;
  return `<div class="opening-toss" role="dialog" aria-modal="true" aria-labelledby="toss-title" tabindex="-1"><div class="toss-card"><div class="eyebrow">${two ? 'HEADS OR TAILS' : 'EQUAL CHANCE FOR EVERY SAGE'}</div><h2 id="toss-title">${two ? 'The opening coin toss' : 'The opening toss'}</h2><div class="toss-coin" style="--toss-delay:-${elapsed}ms;--toss-end:${two && winner.id === back.id ? 1620 : 1440}deg"><div class="coin-face coin-front">${sage(front)}<span>${two ? 'HEADS' : 'ELEMENT'}</span></div><div class="coin-face coin-back">${sage(back)}<span>${two ? 'TAILS' : 'ELEMENT'}</span></div></div><div class="toss-result" style="--result-delay:${Math.max(0,1400-elapsed)}ms" role="status"><strong>${esc(winner.name)} goes first</strong><span>${two ? `${winner.id === front.id ? 'Heads' : 'Tails'} wins the toss.` : 'The starting Sage has been drawn.'}</span></div>${two ? `<div class="coin-assignment"><span>Heads · ${esc(front.name)}</span><span>Tails · ${esc(back.name)}</span></div>` : `<div class="coin-assignment">${g.players.map(p => `<span>${esc(p.name)}</span>`).join('')}</div>`}</div></div>`;
}
function render({ preserveBoard = false } = {}) {
  const previousScreen = app.dataset.screen;
  app.dataset.screen = room ? game() ? 'game' : 'lobby' : 'home';
  ...
  app.insertAdjacentHTML('beforeend', openingMarkup(game()));
  clearTimeout(openingTimer);
  if (openingActive()) {
    openingTimer = setTimeout(() => render(), Math.max(50, game().opening.readyAt - tableNow() + 30));
    if (openingShown !== game().opening.id) { openingShown = game().opening.id; app.querySelector('.opening-toss')?.focus(); playSound('stone'); }
  }
  if (previousScreen !== app.dataset.screen) window.scrollTo(0, 0);
...
app.addEventListener('click', async event => {
  ...
  if ((busy || openingActive()) && button.dataset.do !== 'rules') return;
  try {
    ...
    switch (button.dataset.do) {
      ...
      case 'replay': if (feedbackRoom === room?.code) presentFeedback(lastFeedback); break;
...
document.addEventListener('visibilitychange', () => { if (!document.hidden) poll(); else hush(); });
